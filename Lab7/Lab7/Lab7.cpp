...
// вывод массива
void print_vector(vector<double>& vec, string name, int precision)
{
	cout << "Массив " "\033[94m" <<  name << "\033[97m" ": (";
...
void create_h(vector<double>& h_vec, double a, double b, double h_step, double n)
{
	h_vec.push_back(a); // h_0
	for (size_t i = 0; i < n - 2; ++i)
	{
		a += h_step;
		h_vec.push_back(a); // h_i
	}
	h_vec.push_back(b); // h_n
}
...
int main()
{
	...
	cout << "\033[92m" "Лабораторная работа №7: \"Численное решение ОДУ\"\n\n" "\033[97m";
	...
	// ВАРИАНТ 1
	cout << "Введите шаг разбиения " "\033[94m" "(h)" "\033[97m" " (от 0 до 1.5): ";
	try
	{
		cin >> h;
	...
	// получившееся число точек (на одну меньше от фактического)
	long long m = (b - a) / h; 
	...
	vector<double> inaccuracy_euler(X.size());
	vector<double> inaccuracy_runge_kutta(X.size());
	vector<double> inaccuracy_adams_two(X.size());
	vector<double> inaccuracy_adams_three(X.size());

	// заполнение
	for (size_t i = 0; i < X.size(); ++i)
	{
		inaccuracy_euler[i] = abs(Y_Euler[i] - Y_analytical[i]);
		inaccuracy_runge_kutta[i] = abs(Y_Runge_Kutta[i] - Y_analytical[i]);
		inaccuracy_adams_two[i] = abs(Y_Adams_Two[i] - Y_analytical[i]);
		inaccuracy_adams_three[i] = abs(Y_Adams_Three[i] - Y_analytical[i]);
	}

	// максимальные погрешности при заданном h
	cout << "При " "\033[94m" "h " "\033[97m" "= " << h << ":";
	cout << "\nМаксимальная погрешность " "\033[94m" "Эйлера" "\033[97m" ": " << *max_element(inaccuracy_euler.begin(), inaccuracy_euler.end());
	cout << "\nМаксимальная погрешность " "\033[94m" "Рунге-Кутта" "\033[97m" ": " << *max_element(inaccuracy_runge_kutta.begin(), inaccuracy_runge_kutta.end());
	cout << "\nМаксимальная погрешность " "\033[94m" "Адамса_2" "\033[97m" ": " << *max_element(inaccuracy_adams_two.begin(), inaccuracy_adams_two.end());
	cout << "\nМаксимальная погрешность " "\033[94m" "Адамса_3" "\033[97m" ": "  << *max_element(inaccuracy_adams_three.begin(), inaccuracy_adams_three.end());
